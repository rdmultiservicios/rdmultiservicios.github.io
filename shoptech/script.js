    ...
    featuredProducts.forEach(product => {
        ...
        featuredProduct.innerHTML = `
            ...
            <div class="featured-product-info">
                ...
                <p class="product-description">${product.descripcion}</p>
                <p class="featured-product-price">S/${product.precio.toFixed(2)}</p>
                <div class="product-actions">
                    <button class="btn btn-add" data-id="${product.id}">Añadir</button>
                </div>
            </div>
...
function checkout() {
    ...
    cart.forEach(item => {
        message += `• ${item.nombre} x${item.quantity} - S/${(item.precio * item.quantity).toFixed(2)}\n`;
    });
    const total = cart.reduce((sum, item) => sum + (item.precio * item.quantity), 0);
    message += `\n *Total:* S/${total.toFixed(2)}\n\n`;
    message += 'Por favor, confirma tu pedido.';

    // Número de WhatsApp al que se enviará el pedido (modifica con el tuyo)
    const phoneNumber = '51939975800'; // Sin "+" ni espacios. Ej: Perú = 51999999999
    ...
    productModalBody.innerHTML = `
        <div class="product-details">
            ...
            <div class="product-details-info">
                ...
                <p class="product-details-price">S/${product.precio.toFixed(2)}</p>
